...
func TestCreateXinfinFilterMessage(t *testing.T) {
	tests := []struct {
		name string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := createXinfinManager(tt.p, store.Subscription{Ethereum: tt.args}).GetTriggerJson(); !reflect.DeepEqual(got, tt.want) {
	...
	t.Run("has invalid filter query", func(t *testing.T) {
		...
		got := xinfinManager{ethManager{fq: &filterQuery{BlockHash: &blockHash, FromBlock: "0x1", ToBlock: "0x2"}}}.GetTriggerJson()
		if got != nil {
...
func TestXinfinManager_GetTestJson(t *testing.T) {
	type fields struct {
		fq *filterQuery
		p  subscriber.Type
	}
...
func TestXinfinManager_ParseTestResponse(t *testing.T) {
	type fields struct {
		fq *filterQuery
		p  subscriber.Type
	}
...
func TestXinfinManager_ParseResponse(t *testing.T) {
	type manager struct {
		fq *filterQuery
		p  subscriber.Type
	}
