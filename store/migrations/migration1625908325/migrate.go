...
func Migrate(tx *gorm.DB) error {
	// NOTE auto migration text to text[] column type is not supported
	err := tx.Exec("ALTER TABLE eth_subscriptions ALTER COLUMN topics TYPE text[] USING ARRAY[topics]").Error
	if err != nil {
		return errors.Wrap(err, "failed to migrate topics column")
	}
...
func Rollback(tx *gorm.DB) error {
	return tx.AutoMigrate(&migration0.EthSubscription{}).Error
}
