...
func Migrate(tx *gorm.DB) error {
	...
	err = tx.AutoMigrate(&AlgorandSubscription{}).AddForeignKey("subscription_id", "subscriptions(id)", "CASCADE", "CASCADE").Error
	if err != nil {
		return errors.Wrap(err, "failed to auto migrate AlgorandSubscription")
	}
...
func Rollback(tx *gorm.DB) error {
	return tx.DropTable("algorand_subscriptions").Error
}
