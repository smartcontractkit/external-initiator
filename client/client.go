...
func generateCmd() *cobra.Command {
	...
	newcmd.Flags().Uint("fm_retry_attempts", 3, "The maximum number of attempts that will be made for FM adapter polls")
	must(v.BindPFlag("fm_retry_attempts", newcmd.Flags().Lookup("fm_retry_attempts")))

	newcmd.Flags().Duration("fm_retry_delay", 1*time.Second, "The delay between FM adapter poll attempts")
	must(v.BindPFlag("fm_retry_delay", newcmd.Flags().Lookup("fm_retry_delay")))
